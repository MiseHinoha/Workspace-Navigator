/**
 * Background Script - 后台服务 (Chrome Manifest V3)
 * 处理 API 调用和跨域请求
 */
...
async function getConfig() {
  const result = await chrome.storage.sync.get(['navigatorUrl', 'navigatorToken']);
  return {
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === 'add-to-navigator') {
    // Chrome 不支持 openPopup，只能提示用户点击图标
    console.log('Context menu clicked');
  }
});

console.log('[Workspace Navigator] Background script loaded (Chrome V3)');
